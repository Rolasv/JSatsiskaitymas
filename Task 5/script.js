...
function filterDogOwers (mas){
  mas.forEach(element => {if (element.hasDog === true) console.log(element.name);
  });
}
function filterAdults(mas){
  let newMas = new Array;
  mas.forEach(element => {if (element.age > 17) newMas.push(element.name);
  });
  return newMas;
}
